<script lang="ts">
    ...
    export function tick(dt: number) {
        ...
        for (let i = 0; i < particleCount; i++) {
            pressure[i] =
                pressureStiffness * Math.max(density[i] - densityRest, 0);
        }
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            eachNearbyParticle(x, y, (j) => {
                ...
                // console.log(
                //     `${i} => ${j}\n` +
                //         `${i} pos : ${position[3 * i]}, ${position[3 * i + 1]}\n` +
                //         `${j} pos : ${position[3 * j]}, ${position[3 * j + 1]}\n` +
                //         `delta : ${dx}, ${dy}, r=${r}\n` +
                //         `influence : ${influence}\n` +
                //         `spikyGrad : ${spikyGrad(r, densityRadius)}\n` +
                //         `final addition : ${(-influence * dx) / (r === 0 ? 1 : r)} ${(-influence * dy) / (r === 0 ? 1 : r)}\n` +
                //         `pressure : ${pressure[i]} ${pressure[j]}`,
                // );

                // Add normalized direction
                ax += (-influence * dx) / (r === 0 ? 1 : r);
                ay += (-influence * dy) / (r === 0 ? 1 : r);
            });
